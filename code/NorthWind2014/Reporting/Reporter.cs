...
namespace NorthWind.Reporting
{
    public class Reporter : IReporter
    {
        ...
        {
            // Count is less than 1. Can't fetch 0 or less entries...
            if (count <= 0)
            {
                return new Report<IList<OrdersByTotalPriceDto>, ReportError>() {Data = null, Error = new ReportError(){Message = "Count needs to be larger than 0."}};
            }
        ...
        public Report<IList<ProductsBySaleDto>, ReportError> TopProductsBySale(int count)
        {
            ...
            if (count <= 0)
            {
                return new Report<IList<ProductsBySaleDto>, ReportError>() { Data = null, Error = new ReportError() { Message = "Count needs to be larger than 0." } };
            }
        ...
        public Report<EmployeeSaleDto, ReportError> EmployeeSale(int id)
        {
            ...
            if (id <= 0)
            {
                return new Report<EmployeeSaleDto, ReportError>() { Data = null, Error = new ReportError() { Message = "Ids needs to be 1 or larger" } };
            }
